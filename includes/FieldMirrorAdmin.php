...
final class FieldMirrorAdmin
{
    public static function read(Engine $engine,string $key): array
    {
        ...
        if (!$map || !in_array($map['kind'],['product','order','variant'],true)) { throw new \RuntimeException('Indiquez l\'identité d\'un produit, d\'une déclinaison ou d\'une commande synchronisés.'); }
        if ($map['kind']==='variant') {
            ...
            if (!$parents) { throw new \RuntimeException('Le produit parent de cette déclinaison n\'est pas synchronisé.'); } $map=$parents[0];
    ...
    public static function save(Engine $engine,string $key,array $submitted,string $base): void
    {
        ...
        if ((int)($engine->sql('SELECT GET_LOCK(?,5) AS acquired',[$lock])[0]['acquired']??0)!==1) { throw new \RuntimeException('Champs en cours de modification : réessayez dans un instant.'); }
        try {
            $engine->sql('START TRANSACTION');
            $map=$engine->mapping($key); $current=self::read($engine,$key);
            if (!hash_equals(Protocol::fingerprint($current),$base)) { throw new \RuntimeException('Les champs ont changé depuis leur chargement : rechargez avant d\'enregistrer.'); }
            foreach ($submitted as $id=>$row) {
                if (!array_key_exists($id,$current) || !is_array($row)) { throw new \RuntimeException('Seuls les champs déjà synchronisés se modifient.'); }
                $value=json_decode($row['json']??'null',true,16,JSON_THROW_ON_ERROR);
                if (strlen(json_encode($value,JSON_THROW_ON_ERROR))>65536) { throw new \RuntimeException('Valeur de champ trop volumineuse.'); }
